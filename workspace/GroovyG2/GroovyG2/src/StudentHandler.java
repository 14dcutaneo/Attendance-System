...
public class StudentHandler 
{
	...
	{
		PrintServ.print("StudentHandler.Java public StudentHandler(): STHAND ONLINE"
				+ "");
		...
		if (!stds.exists())
		{
			//stds.
			stds.createNewFile();
			PrintServ.print("StudentHandler.Java public StudentHandler(): First run detected. Force save stHand and pu");
			this.Save();
			Main.pu.save();
		}
		else
				...
				catch (Exception c)
				{
					new Errors.AMDNCError(
							Errors.EHAND.Type.THROWN, //TYPE
									"StudentHandler.java "//CLASS
										+ "StudentHandler() "//METHOD
										+ ""//<<EXTRA>>
									, "tmp is null" //DESCRIPTION
										);
	...
	public Student getStudent(long ID)
	{
		for (int i = 0; i < ars.size(); i++)
		{
			if (ars.get(i).assignmentID == ID)
			{
				PrintServ.print("StudentHandler.java public Student getStudent(long ID): Returning student");
				return ars.get(i);
			}
		}
		PrintServ.print("Returning null");
		return null;
	}
	...
	public void sort()
	{
		...
		ArrayList<Student> arsN = new ArrayList<>();
		
		for (int i = 0; i < ars.size(); i++ )
			arsN.add(this.getStudent(unSort[i]));
	...
	public void Save() throws IOException
	{
		...
		for (int i = 0; i<ars.size(); i++)
		{
			PrintServ.print("StudentHandler Public void Save(): Saving: " + ars.get(i).toString());
